...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    else
    {
        if (isset($_FILES['photo']))
        {
            ...
            {
                $splashlink = file_get_contents($_FILES['photo']['tmp_name']);
                $splashlink = mysql_real_escape_string($splashlink);
...
elseif (isset($_GET['show']))
{
    ...
    $result = mysql_query("SELECT ext, UNIX_TIMESTAMP(image_time), splashlink
                             FROM {$table}
                            WHERE id=$id LIMIT 1");
    ...
    list($ext, $image_time, $splashlink) = mysql_fetch_row($result);
    ...
    header('Content-Length: '.strlen($splashlink));
...
?>
<html><head>
<title>MySQL Blob Image Gallery Example: Tutorial By Mistonline.in</title>
</head>
<body>
<?php
